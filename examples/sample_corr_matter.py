#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Matter Correlation-function demo (z=0.5):
- Build P_J components with FAST-PT
- Call ulpt_power_spectrum with CC=3
- Compare ULPT to Dark Emulator
"""

import numpy as np
import matplotlib.pyplot as plt
import matplotlib.gridspec as gridspec
from matplotlib.ticker import MaxNLocator

from dark_emulator import darkemu
from fastpt.matter_power_spt import (
    P_13_reg, P_22_J
)
from ulptkit import ulpt_power_spectrum, pk_nowiggle  # pk_nowiggle not plotted here but kept for completeness
import mcfit

# -----------------------------
# Figure style (linear axes)
# -----------------------------
plt.rcParams.update({
    "font.family": "Times New Roman",
    "text.usetex": True,        # Set False if LaTeX is not available
    "font.size": 17,
    "axes.labelsize": 19,
    "axes.titlesize": 20,
    "xtick.labelsize": 16,
    "ytick.labelsize": 16,
    "legend.fontsize": 16,
    "figure.titlesize": 22,
    "lines.linewidth": 2.0,
    "axes.linewidth": 1.2,
    "xtick.major.size": 6,
    "ytick.major.size": 6,
    "xtick.major.width": 1.1,
    "ytick.major.width": 1.1,
})

# -----------------------------
# Configuration
# -----------------------------
z = 0.5
k = np.logspace(-4, 2, 600)   # [h/Mpc]; correlation is evaluated via Hankel transform

# -----------------------------
# Cosmology (for emulator)
# -----------------------------
omega_b = 0.02225
omega_cdm = 0.1198
Omega_Lambda = 0.6844
ln10e10As = 3.094
n_s = 0.9645
w = -1.0
cosmo_dark = np.array([omega_b, omega_cdm, Omega_Lambda, ln10e10As, n_s, w])

def get_consistent_h(omega_b, omega_c, Omega_Lambda, omega_nu=0.00064):
    """Return h consistent with physical densities."""
    Omega_m = 1.0 - Omega_Lambda
    return float(np.sqrt((omega_b + omega_c + omega_nu) / Omega_m))

h = get_consistent_h(omega_b, omega_cdm, Omega_Lambda)

# -----------------------------
# Emulator data and linear xi
# -----------------------------
emu = darkemu.base_class()
emu.set_cosmology(cosmo_dark)
P_lin = emu.get_pklin_from_z(k, z)
r, xi_lin = mcfit.P2xi(k)(P_lin)
xi_nl_emu = emu.get_xinl(r, z)

# -----------------------------
# FAST-PT → P_J (one-loop combination for matter)
# -----------------------------
P_window = [0.2, 0.2]
C_window = 0.75
n_pad    = len(k)

P_J_1loop = P_lin + P_22_J(k, P_lin, P_window, C_window, n_pad) + P_13_reg(k, P_lin)

# -----------------------------
# ULPT with P_J (CC=3) and xi(r)
# -----------------------------
# ulpt_power_spectrum returns (k_out, P_ULPT, ...); use P_ULPT only.
_, P_nl_ulpt = ulpt_power_spectrum(k, P_lin, CC=3, P_J=P_J_1loop)
_, xi_nl_ulpt = mcfit.P2xi(k)(P_nl_ulpt)

# -----------------------------
# Figure: top (r^2 xi), bottom (residual [%]) — linear axes
# -----------------------------
fig = plt.figure(figsize=(9.2, 6.8))
gs = gridspec.GridSpec(nrows=2, ncols=1, height_ratios=[2, 1], hspace=0.06)

# Top panel: r^2 xi
ax1 = fig.add_subplot(gs[0, 0])
ax1.plot(r, r**2 * xi_nl_emu,
         "o", ms=5.5, mfc="white", mec="royalblue", mew=1.2, label=r"Emu")
ax1.plot(r, r**2 * xi_nl_ulpt,
         "-", lw=2.2, color="magenta", label=r"ULPT")
ax1.axhline(1.0, color="k", lw=1.0)
ax1.set_xlim(8, 115)
ax1.set_ylim(1, 26)
#ax1.xaxis.set_major_locator(MaxNLocator(7))
ax1.set_xticks([10,30,50,70,90,110])
ax1.yaxis.set_major_locator(MaxNLocator(7))
ax1.grid(True, alpha=0.25)
ax1.set_ylabel(r"$r^2\,\xi(r)$")
ax1.set_title(fr"Matter correlation functions at $z={z}$")
ax1.legend(loc="upper right", frameon=True)
ax1.tick_params(labelbottom=False)

# Bottom panel: percent residuals (ULPT vs Emulator)
ax2 = fig.add_subplot(gs[1, 0], sharex=ax1)
ax2.plot(r, 100.0 * (xi_nl_ulpt/np.maximum(xi_nl_emu, 1e-60) - 1.0),
         color="magenta", lw=2.2, label=r"$\Delta \xi$")
ax2.axhline(0, color="k", lw=1.0)
ax2.fill_between(r, -3, 3, color="forestgreen", alpha=0.18)  # ±3% band
ax2.set_xlim(8, 115)
ax2.set_xticks([10,30,50,70,90,110])
ax2.set_ylim(-5, 5)
#ax2.xaxis.set_major_locator(MaxNLocator(7))
ax2.yaxis.set_major_locator(MaxNLocator(7))
ax2.grid(True, alpha=0.25)
ax2.set_xlabel(r"$r\;[h^{-1}\,\mathrm{Mpc}]$")
ax2.set_ylabel(r"$100\times(\xi_{\rm ULPT}/\xi_{\rm Emu}-1)$ [\%]", fontsize=13)

plt.tight_layout()
# plt.savefig("matter_corr_z0p5_linear.png", dpi=220, bbox_inches="tight")  # optional
plt.show()

